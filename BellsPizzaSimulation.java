...
public class BellsPizzaSimulation {
    ...
    private Queue<Cyclist> queue = new LinkedList<Cyclist>();
    private static final int CYCLISTS = 3;
    ...
    private void getDeliveryPersons(){
        
        for(char a = 1; a <= CYCLISTS; a++){
            queue.offer(new Cyclist("Cyclist "+String.valueOf(a), true));
        }
    ...
    private double run (Queue<Pizza> queue, Scanner logFile) {
        ...
        while (logFile.hasNext()) {
            ...
            Cyclist cyclist = checkAvailability(time); // update cyclist status and check if a cyclist is available to make the delivery

            // if a cyclist (and a pizza) is available then let them deliver the next pizza from the queue
            while(cyclist != null && !queue.isEmpty()){
                ...
                cyclistAvailableAt = time + fullTripTime; // time + fullTriptime;
                cyclist.setTime(cyclistAvailableAt);
                cyclist.set(false);

                // update profit tally with delivery cost
                total -= Pizza.PerMinuteDeliveryCost * fullTripTime;
                cyclist = checkAvailability(time);
    ...
    private Cyclist checkAvailability(int time){

        Cyclist cyclist = null;
        for(Cyclist c : queue){
            if(time >= c.getNextAvailability()){ // check to see if the cyclist has completed the old delivery
                c.set(true);
            }
